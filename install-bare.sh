#!/usr/bin/env bash
# install-bare.sh — 裸跑模式安装 (进阶选项, 不用 Docker)
#
# 主推用 install.sh (Docker 模式). 本档案给想要原生 Node + PM2 跑的用:
#   - 开发者本地 debug 方便
#   - VPS 已经有 pm2 生态不想加 Docker
#   - 特别场景 (如需要 pm2 namespace 跨专案整合)
#
# 用法:
...
echo -e "${c_bold}"
echo "═══════════════════════════════════════════════════"
echo "  tg-monitor-multi · 裸跑模式安装 (进阶选项)"
echo "═══════════════════════════════════════════════════"
echo -e "${c_reset}"
...
echo ""
echo -e "${c_yellow}⚠${c_reset} 主推用 Docker 模式 (bash install.sh). 确定要裸跑?"
echo "  continue / Ctrl+C"
read -r _

for cmd in git curl; do
  command -v "$cmd" >/dev/null || { err "$cmd 未装"; exit 1; }
done

# ─── Node.js via nvm ─────────────────────────────
install_node_via_nvm() {
  log "装 Node $NODE_VERSION via nvm..."
...
echo -e "${c_green}${c_bold}  ✓ 裸跑模式安装完成 · v$VERSION${c_reset}"
echo -e "${c_bold}═══════════════════════════════════════════════════${c_reset}"
echo ""
echo "下一步:"
echo ""
...
echo ""
echo -e "  2. 开机自启: ${c_cyan}pm2 startup${c_reset}"
echo ""
echo -e "  3. 开浏览器: ${c_cyan}http://$VPS_IP:$WEB_PORT/setup${c_reset}"
echo ""
echo "升级 / 回滚会自动侦测裸跑模式:"
