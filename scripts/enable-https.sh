...
if [[ -n "${1:-}" ]]; then
  ...
  if [[ -n "$SUBDOMAIN" ]]; then
    DOMAIN="${SUBDOMAIN}.${IP}.nip.io"
    ok "自动用 nip.io: $DOMAIN (解析到 $IP, 多项目共存不撞)"
  else
    DOMAIN="${IP}.nip.io"
    ok "自动用 nip.io: $DOMAIN (解析到 $IP)"
  fi
fi
...
# ─── 4b. 模式: 现成 Caddy 占着 → 接入模式 ───────────
  ...
  # 4b-2. 写 docker-compose.override.yml 让网络持久化
  #       下次 docker compose up -d (含 --build) 会自动加入外部 Caddy 网络,
  #       不再需要手动 reconnect. 这是扩大部署后的关键 — 不然每次 rebuild 就 502.
  if [[ -n "$EXT_NET" ]]; then
    cat > docker-compose.override.yml <<EOF
# AUTO-GENERATED by scripts/enable-https.sh
# 目的: 把本容器接到外部 Caddy 网络 ($EXT_NET), 让反代能按 container 名找到本服务.
# 不要手改 — 下次 enable-https.sh 会覆写. 删了会导致 rebuild 后 502.
...
networks:
  caddy_shared:
    ...
    name: $EXT_NET
EOF
    ok "已写 docker-compose.override.yml (固定网络 $EXT_NET)"
  fi

  # 4b-3. 找外部 Caddyfile 宿主机路径
